...
def update_config(cfg, args_dict):
    ...
    if args_dict['mode'] == ('train' or 'pred' or 'eval'):
        cfg.cfg_dir = '%s/%s' % (cfg.base_dir, args_dict['cfg_id'] + index)
    else:
...
class Config:
    ...
    def __init__(self, cfg_id, test=False):
        ...
        self.latent_features = cfg['latent_features']
        self.residual_gcns_num = cfg['residual_gcns_num']

        # indirect variable
        self.joint_num = 16 if self.dataset == 'h36m'  else 14 if self.dataset == 'humaneva' else 21 if self.dataset == 'mogaze' else 22 if self.dataset == 'mogaze_withcontext' else 23 if self.dataset == 'gimo' else 24
