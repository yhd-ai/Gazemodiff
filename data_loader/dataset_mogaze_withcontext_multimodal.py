...
class DatasetMogaze_withgaze_multi(Dataset):
    ...
    def prepare_data(self):
        self.data_file = os.path.join('data', 'data_3d_mogaze_withfullcontext_global_highquility_k0.npz')
        self.subjects_split = {'train': ['p1_1', 'p1_2', 'p2_1', 'p4_1', 'p5_1', 'p6_1', 'p6_2'],
                               'test': ['p7_1','p7_3']}
        ...
        self.removed_joints = {}
        self.kept_joints = np.array([x for x in range(22) if x not in self.removed_joints])
    ...
    def process_data(self):
        ...
        self.S1_skeleton = data_o['p1_1']['pick_001_pose_xyz'][:1, self.kept_joints].copy()
            ...
            for action in data_s.keys():
                ...
                if sub not in self.data_candi.keys():
                    ...
                    """self.data_candi[sub] = util.absolute2relative(data_candi, parents=self.skeleton.parents(),
                                                                  invert=True, x0=x0)"""
                    self.data_candi[sub] = data_candi
...
if __name__ == '__main__':
    np.random.seed(0)
    actions = {'WalkDog'}
    dataset = DatasetMogaze_multi('train', actions=actions)
