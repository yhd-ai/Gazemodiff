...
class TemporalDiffusionTransformerDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.ca_block = TemporalCrossAttention(
            latent_dim, mod_dim,num_head, dropout, time_embed_dim)

    def forward(self, x, fuse_gaze, emb):
        x = self.sa_block(x, emb)
        x = self.ca_block(x, fuse_gaze, emb) 
        x = self.ffn(x, emb)
        return x
...
class MotionTransformer(nn.Module):
    def __init__(self,
        ...
        #print(self.input_feats * self.num_frames)
    ...
    def forward(self, x, timesteps, mod=None):
        ...
        #print("emb",emb.shape)
        #print('input dim',self.input_feats * self.num_frames)
        force_mask = mod.get('uncond', False)

        if mod is not None:
            #print("mod",mod.shape)
   
            emb += self.mask_cond(self.cond_embed(mod['y'].reshape(B, -1)), force_mask=force_mask)
