...
def pose_generator(data_set, model_select,gaze_gcn, diffusion, cfg, fr_start,mode=None,
                   action=None, nrow=1):
    """
    ...
    while True:
        ...
        for k in range(0, nrow):
            if mode == 'switch':
                ...
                data = data_set.sample_iter_action(action, cfg.dataset,fr_start)
            ...
            b = traj_dct_mod.shape[0]
            
            traj_dct_mod = gaze_gcn(traj_dct_mod.reshape(b,f,jo,cfg.n_pre)).reshape(b,cfg.n_pre,-1)
            ...
            #traj_est[:,:,-2:-1,:] = traj_est[:,:,-2:-1,:]*0.1
            #print(traj_est[0,0,:,0])
