...
from models.transformer_crossatt import MotionTransformer
from models.diffusion import Diffusion
from models.graph_attention_network_spatialtemporal import graph_convolution_network
...
from data_loader.dataset_mogaze_multimodal import DatasetMogaze_multi
from data_loader.dataset_mogaze_withcontext import DatasetMogaze_withgaze
from data_loader.dataset_mogaze_withcontext_multimodal import DatasetMogaze_withgaze_multi
from data_loader.dataset_gimo import DatasetGimo
from data_loader.dataset_gimo_multimodal import DatasetGimo_multi
from data_loader.dataset_gimo_withcontext import DatasetGimo_withcontext
from data_loader.dataset_gimo_withcontext_multimodal import DatasetGimo_withcontext_multi
...
def create_model_and_diffusion(cfg):
    ...
    """
    n_input_channels=config.scene_feats_dim,
                                                        n_latent=output_len,
    ...
    """
    gaze_gcn = graph_convolution_network(
        in_features=3,
        latent_features=cfg.latent_features,
        node_n=cfg.joint_num,
        seq_len=cfg.n_pre,
        p_dropout=cfg.dropout,
        residual_gcns_num=cfg.residual_gcns_num,
    
    ).to(cfg.device)
    return model, diffusion, gaze_gcn
...
def dataset_split(cfg):
    ...
    """
    dataset_cls = DatasetH36M if cfg.dataset == 'h36m' else DatasetHumanEva if cfg.dataset == 'human_eva' else DatasetMogaze if cfg.dataset == 'mogaze' else DatasetMogaze_withgaze if cfg.dataset == 'mogaze_withcontext' else DatasetGimo if cfg.dataset == 'gimo' else DatasetGimo_withcontext
    
    dataset = dataset_cls('train', cfg.t_his, cfg.t_pred, actions='all')
    dataset_test = dataset_cls('test', cfg.t_his, cfg.t_pred, actions='all')

    dataset_cls_multi = DatasetH36M_multi if cfg.dataset == 'h36m' else DatasetHumanEva_multi if cfg.dataset == 'human_eva' else DatasetMogaze_multi if cfg.dataset == 'mogaze' else DatasetMogaze_withgaze_multi if cfg.dataset == 'mogaze_withcontext' else DatasetGimo_multi if cfg.dataset == 'gimo' else DatasetGimo_withcontext_multi
...
def get_multimodal_gt_full(logger, dataset_multi_test, args, cfg):
    ...
    print(all_data.max(),all_data.min())
