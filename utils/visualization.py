...
def render_animation(skeleton, poses_generator, algos, t_hist, dataset,fix_0=True, azim=0.0, output=None, mode='pred', size=2, ncol=5,
        ...
        if not initialized:
            ...
            for j, j_parent in enumerate(parents):
                ...
                for n, ax in enumerate(ax_3d):
                    ...
                    # else:
                    if j == 21:
                        gaze_dir = pos[j,:] - pos[j_parent,:]
                        gaze_unitdir = gaze_dir / np.linalg.norm(gaze_dir)
                        pos[j,:] = gaze_unitdir* 0.5 + pos[j_parent,:]
            ...
            for j, j_parent in enumerate(parents):
                ...
                for n, ax in enumerate(ax_3d):
                    ...
                    pos = poses[n][i]
                    if dataset == 'gimo' or dataset =='gimo_withcontext':
                        pos =np.roll(pos, shift=1, axis=1)
